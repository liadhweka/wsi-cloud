# 2026-08-17-022535-weka-s1.7-hydrate-mcr256

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.7  ·  **Started (UTC):** 2026-08-17T02:25:38Z
...
```
bash -c $'\n      set -uo pipefail\n      for t in tcga-brca camelyon16; do\n        echo "== aws s3 sync datasets/$t (max_concurrent_requests=256) =="\n        aws s3 sync --region \'ap-northeast-2\' --only-show-errors           "s3://liad-wsi-cloud/datasets/$t/" "/mnt/weka/data/$t/" || exit 1\n      done\n    ' 
```

## Why this run exists

Stage 1.7 cell 4/4: full S3->weka hydration of both dataset prefixes, aws s3 sync, max_concurrent_requests=256. Target wiped pre-cell; the final cell's data is kept and byte-verified. Write cell: cache axis n/a.
