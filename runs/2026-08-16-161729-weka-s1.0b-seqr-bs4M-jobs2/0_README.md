# 2026-08-16-161729-weka-s1.0b-seqr-bs4M-jobs2

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-16T16:17:38Z
...
```
fio --name=seqr-bs4M-jobs2 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=4M --numjobs=2 --iodepth=1 --ioengine=libaio --direct=1 --offset=192G --offset_increment=1440G --size=1440G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 34/36: sequential read, bs=4M jobs=2 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 1, offset 192 GiB), fixed de-ordered cell order. Per-job disjoint slice 1440 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted.
