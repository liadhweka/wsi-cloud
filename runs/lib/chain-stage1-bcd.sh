...
log "=========================="
"$REPO/runs/lib/sweep-stage1-seqr.sh" 2>&1 | tee -a "$CHAIN_LOG"
RC_B=${PIPESTATUS[0]}
...
log "==========================="
"$REPO/runs/lib/sweep-stage1-randw.sh" 2>&1 | tee -a "$CHAIN_LOG"
RC_C=${PIPESTATUS[0]}
...
log "==========================="
"$REPO/runs/lib/sweep-stage1-randr.sh" 2>&1 | tee -a "$CHAIN_LOG"
RC_D=${PIPESTATUS[0]}
...
log "    runs/lib/aggregate-sweep.py 'runs/2026-05-0*-s1.0b-seqr-*'"
log "    runs/lib/aggregate-sweep.py 'runs/2026-05-0*-s1.0c-randw-*'"
log "    runs/lib/aggregate-sweep.py 'runs/2026-05-0*-s1.0d-randr-*'"
