...
ORCH="$REPO/runs/lib/orchestrate-clinical-deployment-stage7.sh"
STREAMING="$REPO/runs/lib/streaming-loop-stage7.sh"
RAW_HELPER="$REPO/runs/lib/read-after-write-stage7.py"
...
INFER_MANIFEST=$REPO/runs/manifests/tcga-brca-full40x-stage4a-format.tsv"
...
all() {
  ...
  tier6_cam16
  echo ""
  echo "=== Stage 7 sweep done. Aggregate with: $REPO/runs/lib/aggregate-stage7-clinical.py ==="
}
