...
WHY this aggregator is separate from 4.C's:
  ...
  - Reuses the four debugged parser idioms from aggregate-stage4c-kvikio.py:
      ...
      * ';' delimiter in sar-cpu.csv + non-DPDK filter (cores 24-31 excluded)
...
def parse_cpu_aggregate_excluding_dpdk(run_dir):
    """Aggregate non-DPDK CPU %busy. DPDK cores = NUMA-0 24-31."""
...
def main():
    if len(sys.argv) < 2:
        pattern = str(Path(__file__).resolve().parent.parent / "2026-*-s5.*")
    else:
        pattern = sys.argv[1]
    ...
    out_path = Path(__file__).resolve().parent.parent / "s5.A-training-summary.csv"
