...
def main():
    pattern = sys.argv[1] if len(sys.argv) > 1 else str(Path(__file__).resolve().parent.parent / "2026-*-s6.D-pipeline-e2e-*")
    ...
    out = Path(__file__).resolve().parent.parent / "s6.D-e2e-summary.csv"
    # Union fieldnames
    fieldnames = []
    seen = set()
    for r in rows:
