# 2026-08-22-054301-weka-s6.B.2-stress-syn-N100000-sz5MB-fp32-n64-random-rep3

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 6.B.2  ·  **Started (UTC):** 2026-08-22T05:43:04Z
...
```
/data/local-nvme/conda-envs/wsi-cucim-2604/bin/python /home/ec2-user/wsi-cloud/scripts/read-feature-files-stage6b.py --corpus-dir /mnt/weka/features-6.B-synthetic/syn-N100000-sz5MB-fp32 --pattern random --n-processes 64 --runtime 600 --ramp 300 --latency-csv /home/ec2-user/wsi-cloud/runs/2026-08-22-054301-weka-s6.B.2-stress-syn-N100000-sz5MB-fp32-n64-random-rep3/per-file-latencies.csv --summary-json /home/ec2-user/wsi-cloud/runs/2026-08-22-054301-weka-s6.B.2-stress-syn-N100000-sz5MB-fp32-n64-random-rep3/file-io-summary.json 
```

## Why this run exists

Stage 6.B.2 cell on fs=weka: corpus=syn-N100000-sz5MB-fp32 pattern=random n_processes=64 ramp=300s steady=600s. WHY: the small-file/metadata substage — structurally NOT bandwidth-bound, so it stays discriminating even under a client-capped ceiling, and it exercises whichever metadata architecture this leg's filesystem uses. Per-file-load latency CSV is the PRIMARY headline source. Cache state recorded as achieved, not asserted (D13). Regime: warm (cache-served) — corpus 488 GiB sits under the 2304 GiB cold floor (Stage-6 register), so server-side residency can serve the steady-state window regardless of the client-side discard at entry; reported as cache-served, never as cold storage throughput. [D18 REP=3 re-invocation of 2026-08-22-034914-weka-s6.B.2-stress-syn-N100000-sz5MB-fp32-n64-random — same command, same env; original-run-dir paths rewritten into this rep's dir; reps group to median + spread]
