# 2026-08-16-165746-weka-s1.0b-seqr-bs1M-jobs8

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-16T16:57:49Z
...
```
fio --name=seqr-bs1M-jobs8 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=1M --numjobs=8 --iodepth=1 --ioengine=libaio --direct=1 --offset=384G --offset_increment=336G --size=336G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 35/36 RE-RUN (original recordings lost to the 2026-08-16 root-volume ENOSPC; original dir forensically renamed -FAILED-enospc-recording): sequential read, bs=1M jobs=8 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass (no time_based; stops at min(slice, 600s)), offset slot 2 (384 GiB), per-job disjoint slice 336 GiB — identical parameters to the driver invocation. Evidence cell: the sweep warm reference. Server-side residual recorded, not asserted.
