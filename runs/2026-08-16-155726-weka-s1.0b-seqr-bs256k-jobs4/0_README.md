# 2026-08-16-155726-weka-s1.0b-seqr-bs256k-jobs4

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-16T15:57:30Z
...
```
fio --name=seqr-bs256k-jobs4 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=256k --numjobs=4 --iodepth=1 --ioengine=libaio --direct=1 --offset=576G --offset_increment=624G --size=624G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 32/36: sequential read, bs=256k jobs=4 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 3, offset 576 GiB), fixed de-ordered cell order. Per-job disjoint slice 624 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted.
