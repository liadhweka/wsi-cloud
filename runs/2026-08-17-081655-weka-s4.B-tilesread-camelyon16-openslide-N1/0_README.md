# 2026-08-17-081655-weka-s4.B-tilesread-camelyon16-openslide-N1

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 4.B  ·  **Started (UTC):** 2026-08-17T08:16:58Z
...
```
env CONDA_PREFIX=/data/local-nvme/conda-envs/wsi-cucim /data/local-nvme/conda-envs/wsi-cucim/bin/python /home/ec2-user/wsi-cloud/scripts/read-tiles-onthefly.py --backend openslide --n-processes 1 --svs-dir /mnt/weka/data/camelyon16/images --coords-dir /mnt/weka/tissue-detection/3.0/camelyon16/n64/patches --runtime 60 --ramp 10 --coord-pool-pickle /data/local-nvme/stage4b-pool-cache/camelyon16.pkl --latency-csv /tmp/stage4b-latencies/tilesread-camelyon16-openslide-N1.csv --summary-json /tmp/stage4b-latencies/tilesread-camelyon16-openslide-N1.summary.json --seed 42 
```

## Why this run exists

Stage 4.B tier1 cell 15/26: random tile reads. Backend=openslide (per-tile via multiprocessing.Pool). Dataset=camelyon16 (full coord pool from CLAM 3.0 n64 outputs). N_processes=1. LRU(8) slide handle cache per worker. seed=42. runtime=60s + 10s ramp. Cache arm=warm. Cell order fixed and de-ordered in N (D13).
