# 2026-08-17-112200-weka-s1.0b-seqr-bs4M-jobs64-rep2

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-17T11:22:03Z
...
```
fio --name=seqr-bs4M-jobs64 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=4M --numjobs=64 --iodepth=1 --ioengine=libaio --direct=1 --offset=0G --offset_increment=48G --size=48G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 17/36: sequential read, bs=4M jobs=64 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 0, offset 0 GiB), fixed de-ordered cell order. Per-job disjoint slice 48 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted. [D18 REP=2 re-invocation of 2026-08-16-134518-weka-s1.0b-seqr-bs4M-jobs64 — same command, same env; reps group to median + spread]
