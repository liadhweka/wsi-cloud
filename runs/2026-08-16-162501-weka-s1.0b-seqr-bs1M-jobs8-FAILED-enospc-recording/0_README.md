# 2026-08-16-162501-weka-s1.0b-seqr-bs1M-jobs8

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-16T16:25:04Z
...
Stage 1.0b cell 35/36: sequential read, bs=1M jobs=8 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 2, offset 384 GiB), fixed de-ordered cell order. Per-job disjoint slice 336 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted.
