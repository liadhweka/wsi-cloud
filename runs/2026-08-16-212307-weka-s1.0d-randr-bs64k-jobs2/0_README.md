# 2026-08-16-212307-weka-s1.0d-randr-bs64k-jobs2

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0d  ·  **Started (UTC):** 2026-08-16T21:23:10Z
...
```
fio --name=randr-bs64k-jobs2 --filename=/mnt/weka/benchmarks/stage1-read-corpus/region-5.bin --rw=randread --bs=64k --numjobs=2 --iodepth=8 --ioengine=libaio --direct=1 --offset_increment=128G --size=128G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0d cell 6/22: random read IOPS, bs=64k jobs=2 iodepth=8 libaio --direct=1, region-5.bin (256 GiB; per-job disjoint 128 GiB slices, fio random map on => each block at most once). COLD BY CONSTRUCTION (one-touch: no block of this region has been read before, staging warmth evicted by the prep's flush pass). Fixed de-ordered cell order; stops at min(one-touch complete, 600s). Server-side residual recorded, not asserted.
