# 2026-08-17-113636-weka-s1.0b-seqr-bs4M-jobs4-rep3

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-17T11:36:39Z
...
```
fio --name=seqr-bs4M-jobs4 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=4M --numjobs=4 --iodepth=1 --ioengine=libaio --direct=1 --offset=384G --offset_increment=672G --size=672G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 11/36: sequential read, bs=4M jobs=4 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 2, offset 384 GiB), fixed de-ordered cell order. Per-job disjoint slice 672 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted. [D18 REP=3 re-invocation of 2026-08-16-125802-weka-s1.0b-seqr-bs4M-jobs4 — same command, same env; reps group to median + spread]
