# 2026-08-23-124753-weka-s1.6-fio-scratch-layout-prep

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.6  ·  **Started (UTC):** 2026-08-23T12:47:56Z
...
```
fio --name=fio-scratch-layout --directory=/mnt/weka/benchmarks/fio-scratch-mixed --filename_format=fio-scratch-mixed.\$jobnum.\$filenum --rw=write --bs=1M --size=4G --numjobs=64 --iodepth=1 --ioengine=libaio --direct=1 --create_only=0 --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.6 PREP (not a sweep cell): pre-stage 64x4G fio scratch files at /mnt/weka/benchmarks/fio-scratch-mixed/ for the read side of the mixed sweep. Created once, reused across all cells (avoids per-cell layout phase). Side measurement: 256 GB sequential write at moderate concurrency, a 1.0a cross-check on real-world write.
