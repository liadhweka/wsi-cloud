# 2026-08-15-182302-weka-s0-probe-clientcap-seqw-bs4m-jobs16

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 0  ·  **Started (UTC):** 2026-08-15T18:23:05Z
...
```
fio --name=clientcap --directory=/mnt/weka/benchmarks/fio-scratch --rw=write --bs=4M --size=10G --numjobs=16 --iodepth=8 --ioengine=libaio --direct=1 --runtime=300 --time_based --group_reporting --output-format=json+ 
```

## Why this run exists

DIAGNOSTIC PROBE, never quote: client-capability seq WRITE, bs=4M numjobs=16 iodepth=8, 300s. Decision support for the backend-sizing question (memory item 22): does the 4-FE-core client exceed the 8x i8ge.2xlarge backends' 67 Gbps sustained aggregate? Files retained for the read probe.
