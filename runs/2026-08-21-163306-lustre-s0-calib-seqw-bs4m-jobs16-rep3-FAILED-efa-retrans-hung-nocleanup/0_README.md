# 2026-08-21-163306-lustre-s0-calib-seqw-bs4m-jobs16-rep3

**Filesystem:** lustre (mounted at /mnt/lustre)
**Stage:** 0  ·  **Started (UTC):** 2026-08-21T16:33:07Z
...
```
fio --name=calib-seqw-bs4m-jobs16-rep3 --directory=/mnt/lustre/benchmarks/fio-canary-calib --filename_format=calib.\$jobnum --size=10G --numjobs=16 --ioengine=libaio --direct=1 --iodepth=8 --time_based --group_reporting --output-format=json+ --status-interval=1 --rw=write --bs=4M --runtime=300 
```

## Why this run exists

CANARY-BAND CALIBRATION cell (D-5), diagnostic, never quote: calib-seqw-bs4m-jobs16-rep3. Probe-shaped fio against /mnt/lustre/benchmarks/fio-canary-calib; wire/app ratio feeds runs/.leg-state/lustre/canary-bands.json via wsi_agg_helper.py calibrate. Read cells are deliberately backend-RAM-resident (drives out of the picture; the ratio is the measurement).
