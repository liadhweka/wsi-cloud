# 2026-08-16-143158-weka-s1.0b-seqr-bs64k-jobs16

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0b  ·  **Started (UTC):** 2026-08-16T14:32:01Z
...
```
fio --name=seqr-bs64k-jobs16 --filename=/mnt/weka/benchmarks/stage1-read-corpus/seq-corpus.bin --rw=read --bs=64k --numjobs=16 --iodepth=1 --ioengine=libaio --direct=1 --offset=192G --offset_increment=180G --size=180G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0b cell 22/36: sequential read, bs=64k jobs=16 iodepth=1 libaio --direct=1. COLD BY CONSTRUCTION (D13 route 1): pre-staged 3072 GiB scan corpus (>= ~2x the larger server cache; cyclic-scan LRU self-eviction), single pass per cell (no time_based; stops at min(slice, 600s)), per-cell offset rotation (slot 1, offset 192 GiB), fixed de-ordered cell order. Per-job disjoint slice 180 GiB. Evidence cell: the sweep's warm reference. Server-side residual recorded, not asserted.
