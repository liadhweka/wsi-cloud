# 2026-08-17-123748-weka-s1.0d-randr-bs4k-jobs32-rep3

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0d  ·  **Started (UTC):** 2026-08-17T12:37:51Z
...
```
fio --name=randr-bs4k-jobs32 --filename=/mnt/weka/benchmarks/stage1-read-corpus/region-24.bin --rw=randread --bs=4k --numjobs=32 --iodepth=8 --ioengine=libaio --direct=1 --offset_increment=8G --size=8G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0d D18 REP=3 re-invocation of randr-bs4k-jobs32 (the knee cell): FRESH reserve region-24.bin claimed via the ledger (a repeat re-reading its first run region measures that run cache, not storage). bs=4k jobs=32 iodepth=8 libaio --direct=1, per-job disjoint 8 GiB slices, fio random map => each block at most once; stops at min(one-touch, 600s). Driver-identical parameters.
