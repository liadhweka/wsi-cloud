# 2026-08-22-083244-weka-s1.5-restage-tcga-brca-for-1.5-prep-n16

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.5  ·  **Started (UTC):** 2026-08-22T08:32:48Z
...
```
/usr/bin/fpsync -v -n 16 -d /tmp/fpsync-prep /mnt/weka/data/tcga-brca/ /data/local-nvme/fpsync-source/tcga-brca/ 
```

## Why this run exists

Recorded re-stage (the 1.5 driver's own documented prep): fpsync -n 16 /mnt/weka/data/tcga-brca/ -> local NVMe fpsync-source. WHY: 1.5 measures local->filesystem writes, so the corpus must pre-exist on the fast local source; 6.C/7.5's ingest workloads read the same staged source. A staging pass, not a comparison cell (na-staging-prep); mount is idle (nothing in flight).
