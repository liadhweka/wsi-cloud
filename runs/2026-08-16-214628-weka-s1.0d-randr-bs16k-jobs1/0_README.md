# 2026-08-16-214628-weka-s1.0d-randr-bs16k-jobs1

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0d  ·  **Started (UTC):** 2026-08-16T21:46:31Z
...
```
fio --name=randr-bs16k-jobs1 --filename=/mnt/weka/benchmarks/stage1-read-corpus/region-15.bin --rw=randread --bs=16k --numjobs=1 --iodepth=8 --ioengine=libaio --direct=1 --offset_increment=256G --size=256G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0d cell 16/22: random read IOPS, bs=16k jobs=1 iodepth=8 libaio --direct=1, region-15.bin (256 GiB; per-job disjoint 256 GiB slices, fio random map on => each block at most once). COLD BY CONSTRUCTION (one-touch: no block of this region has been read before, staging warmth evicted by the prep's flush pass). Fixed de-ordered cell order; stops at min(one-touch complete, 600s). Server-side residual recorded, not asserted.
