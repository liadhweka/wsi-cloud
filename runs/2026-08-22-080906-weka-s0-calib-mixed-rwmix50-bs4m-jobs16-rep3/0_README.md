# 2026-08-22-080906-weka-s0-calib-mixed-rwmix50-bs4m-jobs16-rep3

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 0  ·  **Started (UTC):** 2026-08-22T08:09:09Z
...
```
fio --name=calib-mixed-rwmix50-bs4m-jobs16-rep3 --directory=/mnt/weka/benchmarks/fio-canary-calib --filename_format=calib.\$jobnum --size=10G --numjobs=16 --ioengine=libaio --direct=1 --iodepth=8 --time_based --group_reporting --output-format=json+ --status-interval=1 --rw=rw --rwmixread=50 --bs=4M --runtime=300 
```

## Why this run exists

CANARY-BAND CALIBRATION cell (D-5), diagnostic, never quote: calib-mixed-rwmix50-bs4m-jobs16-rep3. Probe-shaped fio against /mnt/weka/benchmarks/fio-canary-calib; wire/app ratio feeds runs/.leg-state/weka/canary-bands.json via wsi_agg_helper.py calibrate. Read cells are deliberately backend-RAM-resident (drives out of the picture; the ratio is the measurement).
