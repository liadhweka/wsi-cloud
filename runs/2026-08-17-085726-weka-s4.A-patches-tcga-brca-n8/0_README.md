# 2026-08-17-085726-weka-s4.A-patches-tcga-brca-n8

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 4.A  ·  **Started (UTC):** 2026-08-17T08:57:29Z
...
```
env CONDA_PREFIX=/data/local-nvme/conda-envs/wsi-cucim /data/local-nvme/conda-envs/wsi-cucim/bin/python /home/ec2-user/wsi-cloud/scripts/extract-tiles-to-hdf5.py --concurrency 8 --svs-dir /mnt/weka/data/tcga-brca --coords-dir /mnt/weka/tissue-detection/3.0/tcga-brca/n64/patches --output-dir /mnt/weka/patches/4.A/tcga-brca/n8 --latency-csv /tmp/stage4a-latencies/tcga-brca-n8-latencies.csv --manifest /home/ec2-user/wsi-cloud/scripts/manifests/tcga-brca-stage4a-subset.tsv 
```

## Why this run exists

Stage 4.A cell 3/6: pre-extract tiles to per-slide HDF5. Dataset=tcga-brca (50-slide random subset, seed=42, manifest=/home/ec2-user/wsi-cloud/scripts/manifests/tcga-brca-stage4a-subset.tsv, 50 slides), concurrency=8. Per slide: openslide.OpenSlide + read_region(level=0, 256x256) for each CLAM coord (~52K BRCA / ~11K CAMELYON16 avg per slide), JPEG-encoded q=85, written to /mnt/weka/patches/4.A/tcga-brca/n8/<slide-id>.h5 with variable-length JPEG bytes dataset + coords + attrs. Single-pass. Concurrency via Python multiprocessing.Pool (per-slide-level).
