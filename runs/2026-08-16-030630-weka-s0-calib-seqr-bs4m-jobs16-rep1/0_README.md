# 2026-08-16-030630-weka-s0-calib-seqr-bs4m-jobs16-rep1

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 0  ·  **Started (UTC):** 2026-08-16T03:06:33Z
...
```
fio --name=calib-seqr-bs4m-jobs16-rep1 --directory=/mnt/weka/benchmarks/fio-canary-calib --filename_format=calib.\$jobnum --size=10G --numjobs=16 --ioengine=libaio --direct=1 --iodepth=8 --time_based --group_reporting --output-format=json+ --status-interval=1 --rw=read --bs=4M --runtime=480 
```

## Why this run exists

CANARY-BAND CALIBRATION cell (D-5), diagnostic, never quote: calib-seqr-bs4m-jobs16-rep1. Probe-shaped fio against /mnt/weka/benchmarks/fio-canary-calib; wire/app ratio feeds runs/.leg-state/weka/canary-bands.json via wsi_agg_helper.py calibrate. Read cells are deliberately backend-RAM-resident (drives out of the picture; the ratio is the measurement).
