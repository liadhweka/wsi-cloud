# 2026-08-18-153904-weka-s4.D-rawtiff-brca-par4

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 4.D  ·  **Started (UTC):** 2026-08-18T15:39:07Z
...
```
/home/ec2-user/wsi-cloud/scripts/convert-stage4c-rawtiff.sh --inner brca /tmp/tmp.pQu0mCNnsK /home/ec2-user/wsi-cloud/runs/2026-08-18-153904-weka-s4.D-rawtiff-brca-par4 
```

## Why this run exists

Stage 4.D: TRUE 20x raw-TIFF conversion of brca (1064 slides, PARALLEL=4 — workload shape, must match on both legs). Sustained large-read (canonical source) + large-write (single-level 256px-tiled uncompressed 20x TIFF, D4) via convert-rawtiff-20x.py; per-dataset read params per the coord contract (CAM16 native L1@256; BRCA 512@40x resized to 256). Fail-loud mpp guard; write-to-.partial-then-rename; idempotent skip on existing non-empty output (SKIP-EXISTS is a distinct status — verify cleanup before regenerating, RUNBOOK silent-skip hazard). Artifact RETAINED at rest (Stage-4 register). Per-invocation log: conversion-log.tsv in this run dir.
