# 2026-08-21-204248-lustre-s0-efa-bulk-repro-120s

**Filesystem:** lustre (mounted at /mnt/lustre)
**Stage:** 0  ·  **Started (UTC):** 2026-08-21T20:42:49Z
...
```
fio --name=efa-bulk-repro --directory=/mnt/lustre/benchmarks/fio-canary-calib --filename_format=calib.\$jobnum --size=10G --numjobs=16 --ioengine=libaio --direct=1 --iodepth=8 --rw=write --bs=4M --runtime=120 --time_based --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

EFA bulk-write incident reproduce probe (120s, 16j x 4M direct — the incident shape). DIAGNOSTIC, never quote a rate: the subject is transport stability, evidenced by the EFA retrans counters and dmesg across the cell. Incident evidence: runs/2026-08-21-1*-FAILED-efa-*.
