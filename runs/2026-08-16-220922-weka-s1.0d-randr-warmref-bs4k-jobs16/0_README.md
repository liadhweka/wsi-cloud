# 2026-08-16-220922-weka-s1.0d-randr-warmref-bs4k-jobs16

**Filesystem:** weka (mounted at /mnt/weka)
**Stage:** 1.0d  ·  **Started (UTC):** 2026-08-16T22:09:25Z
...
```
fio --name=randr-warmref-bs4k-jobs16 --filename=/mnt/weka/benchmarks/stage1-read-corpus/region-20.bin --rw=randread --bs=4k --numjobs=16 --iodepth=8 --ioengine=libaio --direct=1 --offset_increment=16G --size=16G --runtime=600 --ramp_time=60 --group_reporting --output-format=json+ --status-interval=1 
```

## Why this run exists

Stage 1.0d cell 22/22: random read IOPS, bs=4k jobs=16 iodepth=8 libaio --direct=1, region-20.bin (256 GiB; per-job disjoint 16 GiB slices, fio random map on => each block at most once). WARM REFERENCE (D13 route 2, inverted — the grid's default regime is cold): deliberate re-read of the last grid cell's just-touched region. Fixed de-ordered cell order; stops at min(one-touch complete, 600s). Server-side residual recorded, not asserted.
