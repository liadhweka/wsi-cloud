...
carry. It now has both: `cloud-setup/HANDOFF-NEXT-SESSION.md`, gated by `runs/lib/teardown-preflight.sh`.
...
- **`runs/Stage-<N>-*.md`** for every stage touched — results, what each substage answered, the decision and
  change logs. This is the audit trail.
- **`PRESENTING.md`** — any finding or caveat that landed. **Keep single-leg claims scoped as half an
  unfinished comparison**, and leave `[STORY PENDING RESULTS]` where results genuinely don't exist yet.
- **`runs/STAGES.md`** — stage statuses, and any project-wide methodology decision (with its *why* and its
  sources).
...
## Step 3 — Write `cloud-setup/HANDOFF-NEXT-SESSION.md`
...
```bash
grep -E '^(Written|Leg):' cloud-setup/HANDOFF-NEXT-SESSION.md
```
...
```bash
source cloud-setup/env.sh
runs/lib/teardown-preflight.sh                 # never --quick before a real teardown
```
...
```bash
runs/lib/env-contract.py show --file "runs/env-contract-leg-$LEG.json" | grep -E 'ami_id|instance_type|aws_region|aws_az|kernel'
```
...
Destruction is theirs. The rebuild path is `cloud-setup/TEARDOWN-AND-REBUILD.md` § Rebuild.
