...
def main():
    if len(sys.argv) < 2:
        ...
        pattern = str(Path(__file__).resolve().parent.parent / "runs" / f"2026-*-{_LEG}-s4.C-*")
    else:
        pattern = sys.argv[1]
    ...
    out_path = Path(__file__).resolve().parent.parent / "runs" / f"s4.C-kvikio-summary-{_LEG_OUT}.csv"
