...
| **The matched artifact is implemented and evidenced (2026-08-23)** | The 7.4.b writer is invoked with `--bytes-per-write 6,440,000` — the mean of the 50 recorded tiff5x heatmap writes in the measured 7.3.a cell on this leg (median 6.19 MB, range 1.3–14.6 MB; cell named in the run note) — and its 256×256 single-level tile geometry matches the measured artifact by construction; the writer records target-vs-achieved so the match is checked at run time, and the poll interval is 1 ms with the resolution floor recorded. **The per-slide unlink stays** — the reader's measurement *is* the non-existent → visible transition, so D13's do-not-recreate-between-cells mechanic must not be swept across this cell. Leg B re-derives its own matched size from its own 7.3 cell |
