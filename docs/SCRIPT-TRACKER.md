...
| **D-35** | **Local raw telemetry fills the 48 GB root volume mid-leg — relocate-after-verified-sync belongs in the chain** | `run-leg.sh` (per-step sync), terraform (next rebuild) | The 2026-08-16 ENOSPC abort: `runs/*/raw` accumulates locally (~16 GB by mid-1.0b) although S3 already held every byte via the per-step verified sync, and the HF model cache (13 GB) also lived on the root volume. Interim (this leg): the session chain relocates each finished step's raw payloads to `/data/local-nvme/runs-raw-overflow/` with symlinks back — nothing deleted, S3 authoritative, parsers still resolve. Durable: fold relocate-after-verified-sync into `run-leg.sh`'s per-step sync, point the HF cache at scratch in the bootstrap, and size the root volume ≥100 GB at the next rebuild (terraform, human) |
