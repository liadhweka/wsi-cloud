...
| **D-16** | **Lustre client-side EFA configuration — RESOLVED (2026-08-20, gated walk → baked)** | `wsi-lustre-phase2.sh`, `run-leg.sh` | The hazard: an EFA-enabled FS mounts and runs happily over tcp (the MGS NID is `@tcp` by design), forfeiting the per-server-cap escape — the load-bearing EFA benefit on this client class (GDS is separately unreachable on g6e, **D8**) — while producing believable numbers, breaking "Lustre at maximum" (**D7**) invisibly. Now enforced mechanically at three layers: phase-2's hard gate (`lnetctl net show` must list an `efa` net or no mount, no fallback), phase-2's LNet **counter-proof** (direct-I/O write must move efa `send_count` ~1 RPC/MiB or it unmounts), and `run-leg.sh` refusing `FS_TRANSPORT` unset-or-tcp without a written waiver. The standing rule: a TCP mount is a human decision, in writing. Reasoning + register: `cloud-setup/LUSTRE-PROVISIONING.md` (in `docs/`) |
| **D-17** | **Kernel half RESOLVED on the validated path (2026-08-20); NVIDIA stack: `nvidia-driver` AND `nvidia-fs` pinned to the contract (nvidia-fs pin ratified 2026-08-21); toolkit minor unpinned — contract verify is the tripwire for the rest** | `bootstrap-instance.sh` | **Resolved by construction:** on AL2023 the Lustre kernel modules (incl. `kefalnd`) ship in the kernel itself and `lustre-client` is userspace-only from the base repo, so the client install cannot move the kernel — and `wsi-lustre-phase2.sh` asserts `uname -r` unchanged across the install (the tripwire if that ever stops being true; a firing is a stop-and-decide with the human, never a silent upgrade). **NVIDIA stack as built:** the client AMI is pinned in terraform (`client_instance_ami_id`, 2026-08-15) and the bootstrap pins `nvidia-driver` to Leg A's contract-recorded `driver_version` (S3 copy), falling back to latest only when that NVR is unavailable — the fallback raises `/var/lib/wsi-DRIVER-DRIFT-CHECK-ME` plus a WARN. The pin fired clean on the 2026-08-21 Leg-B rebuild (595.71.05, contract match). **`nvidia-fs` is pinned the same way** (ratified + implemented 2026-08-21): the contract's `nvidia_fs_version` NVR, falling back to latest with the same drift flag + WARN — proven at the next rebuild. **Unpinned remainder:** the CUDA toolkit minor inside the 12-9 stream, and the no-contract-in-S3 first-build path (nothing to pin to, by construction). The contract verify (kernel/AMI/driver/nvidia-fs are MUST_MATCH) is the tripwire that catches any such drift before a measured cell |
