...
| **D-30** | **Verdict semantics DECIDED + wrapper half DONE (ratified 2026-08-16; register D21): cache_state undeclared on a stage≥1 cell → INCOMPLETE; `RECORD_KVIKIO_CELL=1` without a recorded `path_accounting` split → INCOMPLETE; missing cost inputs → warn only (re-derivable arithmetic); contract-verified marker absent at leg start → refuse. Remaining: the 6.C and 7.5 mixed-cell regimes** | `sweep-stage6c.sh`; `sweep-stage7-clinical.sh` (7.5 cells only) | The wrapper refuses undeclared cells. Done: stage-3, 4.A, 4.D, **4.C** (`cold` — client-cold at read/window entry, reader evidence), **5** (`warm` — steady-state by construction, note-worded), **6.A** (Tier 1/3 per cell; Tier 2 `na-mixed-rw-chunk-resident`, Stage-6 register), **6.B** (`na-write-cell` on generation; 6.B.2 per-cell `cold`/`warm` derived mechanically from the corpus name against the 2304 GiB cold floor, reader-summary evidence via the reconciler; 6.B.3 `warm` steady-state note-worded), **7.1/7.2/7.3/7.4/7.6** (the roadmap's named regimes; 7.4.b `na-visibility-consistency-cell`; kvikIO cells declare `RECORD_KVIKIO_CELL=1`, which fails loud until the Stage-7 worker's accounting wiring lands — see D-6). Remaining: **6.C cells and 7.5's mixed multi-workload cells declare nothing** — their roadmaps define no cache-discipline row, so the regime is an open methodology call (surfaced to the human; the open-items memory carries it). Wire both drivers in the edit that lands the ratified value |
