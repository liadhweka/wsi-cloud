...
| **D-17** | **Kernel half RESOLVED on the validated path (2026-08-20); NVIDIA stack: `nvidia-driver` pinned to the contract, `nvidia-fs` + toolkit minor unpinned — contract verify is the accepted tripwire** | `bootstrap-instance.sh` | **Resolved by construction:** on AL2023 the Lustre kernel modules (incl. `kefalnd`) ship in the kernel itself and `lustre-client` is userspace-only from the base repo, so the client install cannot move the kernel — and `wsi-lustre-phase2.sh` asserts `uname -r` unchanged across the install (the tripwire if that ever stops being true; a firing is a stop-and-decide with the human, never a silent upgrade). **NVIDIA stack as built:** the client AMI is pinned in terraform (`client_instance_ami_id`, 2026-08-15) and the bootstrap pins `nvidia-driver` to Leg A's contract-recorded `driver_version` (S3 copy), falling back to latest only when that NVR is unavailable — the fallback raises `/var/lib/wsi-DRIVER-DRIFT-CHECK-ME` plus a WARN. The pin fired clean on the 2026-08-21 Leg-B rebuild (595.71.05, contract match). **Unpinned remainder:** `nvidia-fs` (`NVIDIA_FS_VERSION`; 2.29.4 matched across builds by repo state, not by pin) and the CUDA toolkit minor inside the 12-9 stream. The contract verify (kernel/AMI/driver/nvidia-fs are MUST_MATCH) is the tripwire that catches any such drift before a measured cell; pinning `nvidia-fs` to an explicit NVR stays a proposal awaiting the human's ratification |
