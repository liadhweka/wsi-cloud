...
| **D-38** | **Ten aggregators lack the FAILED-dir exclusion** — a forensic `-FAILED-<reason>` suffix does not escape their prefix globs, so a renamed dir with a parsed `results.json` re-enters the summary as a zero/garbage row (bit 4.C on 2026-08-18; fixed there, and `aggregate-stage6a-extract.py`/`aggregate-stage1-hydrate.py`/`aggregate-stage6b.py` already exclude) | `aggregate-stage{1-mixed,1-fpsync,2-properties,3-tissue-detection,4a-patches,4b-tilesread,6c-concurrent,7-clinical}.py`, `aggregate-sweep.py` | Past summaries are gate-verified CLEAN (earlier FAILED dirs had no parseable results, and `aggregate-sweep.py`'s name regex rejects the suffix), so no recorded data is polluted — apply the exclusion when each is next touched, or in one pass before Leg B, same policy as the abort-on-failure retrofit |
...
(the `bootstrap-instance.sh` entry's caveat).
