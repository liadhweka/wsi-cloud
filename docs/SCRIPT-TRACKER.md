...
## ⚠️ Deferred work — every row gates its own milestone

None of this could be done meaningfully before the environment existed. **Each row names its own gate** —
"before 6.B", "before Leg B's stage 1", "before stage 7" — and the row's work must land before its gate
or that gate's cells are invalid or refused. This is NOT a single prerequisite list for the first cell; it
is a rolling backlog audited against stage position. Resolved rows are deleted (git is the audit trail;
their standing constraints move to the owning doc or script entry) — **a row in this table is live work.**
Sessions close rows opportunistically: whoever touches a row's scope does the row, in the same edit.
...
now, so citations in scripts and entries never dangle (the D-26 lesson): **D-34** → per-cell `RECORD_POLL_HZ` (1|2|5|10|20; default 1) drives every filesystem-side polling loop in `record-run.sh` (ms timestamps throughout, so parse-results' dt-derived rates and the per-timestamp client summing stay correct at any rate; recorded as metadata `poll_hz`); the 2.0 driver sets 10 per cell; the recorded 1-vs-10 Hz same-config proof pair (`s0-d34-pollrate-proof-*`, 2026-08-22) shows app rates agreeing to 0.03% — no perturbation — with ~5.6 Hz ACHIEVED at HZ=10 (the `weka stats` CLI's own call latency bounds the loop; the achieved interval is in the timestamps). sar stays at 1 s (integer-second tool; not a short-cell primary) · **D-33** → both halves fixed
(2026-08-22): the stage-7 sub-tier grids match on the RECORDED `stage` field first with the name prefix as
fallback (`_in_subtier`; either naming shape lands), and RDMA devices are DISCOVERED from the recorded CSV
and summed per timestamp — hw_counters preferred, port counters the fallback — in both
`aggregate-stage7-clinical.py` and `aggregate-stage6b.py` (no named device anywhere; verified empty-clean
on Leg A, exercised for real on Leg B's EFA NICs) · **D-8** → derived on the real
...
bootstrap on both legs (its §6.5); the checklist rewrite is the `GDS-TUNING-CHECKLIST.md` entry's DEFER ·
**D-11** → register L2/L4 (`cloud-setup/LUSTRE-PROVISIONING.md`), re-applied per boot by
`wsi-lustre-tuning.service` · **D-16** → three-layer mechanical enforcement; `LUSTRE-PROVISIONING.md` + the
`wsi-lustre-phase2.sh` entry · **D-17** → kernel: in-kernel Lustre modules + phase-2's uname assertion;
NVIDIA: contract-pinned driver+nvidia-fs with the toolkit-minor remainder on the contract-verify tripwire
(the `bootstrap-instance.sh` entry's caveat) · **D-36** → resume-skip built into the four 1.0 drivers
(`cell_done_ok`: INDEX-verdict-keyed, leg+stage-anchored, REP bypassed; the randr warm reference carries a
recorded resume caveat when its target region was touched by a prior invocation) — see the sweep drivers'
entry · **D-38** → the FAILED-dir exclusion applied across every aggregator's dir selection (forensic
`-FAILED-<reason>` renames are history, not subjects — same rule as `verify-substage-closeout.sh`) ·
**D-25** → the 6.C GPU partition is ratified (2026-08-21): extract on GPUs 1–3, MIL pinned to GPU 0;
...
missing artifact → refusal), `features-6a` (per (model, dataset): file count, per-slide tile count, tensor
shape + dtype — never values; slides enumerated from the same manifests the extraction cells consumed;
refuses on a header-vs-tensor count mismatch, which downstream loads cannot detect).
...
skipped — *a leg with a hole in it looks complete in `INDEX.md`*, which is the failure this prevents.
Step 6.B.1 runs `sweep-stage6b-stress.sh prep` (the recorded corpus-suite write; grid fixed 2026-08-21).
...
(the fs stays unmounted by design) and the motd's step 3 flips to the triage path. **Contract-at-boot
(lustre only):** the background env build's last act writes the leg's contract and verifies it against
Leg A's committed one — gated on `wsi-lustre-phase2.service` being active, because it runs only once every
input it verifies exists (mount facts from phase-2, python/conda fields from the built envs). A clean verify
arms `runs/.leg-state/lustre/contract-verified` with no session step; any failure leaves **no marker** and
`run-leg.sh` refuses the leg (`D-30`).
...
so the lustre check lives at the end of the env build instead (above). **The rebuild's env.sh merge runs the
contract emit FIRST (`env --for-leg $LEG`, so same-leg fields arrive live) and applies this instance's freshly-derived cluster
...
**All four are resume-skip (D-36):** a cell whose exact name already has an OK-verdict run dir for this
leg+stage is skipped (`cell_done_ok`, keyed on the `INDEX.md` verdict; `-repN` and `-FAILED-*` never match
the anchored glob, and a `REP` invocation never skips), so re-running the driver re-does only what is
missing. The randr warm reference notes when a resume weakened its warm premise (target region touched by a
prior invocation).
...
therefore a **cross-leg integrity check**. The reserved-core exclusion set is measured and contract-recorded
on both legs (weka `1-4,49-52`, lustre `none`).
...
**⏳ DEFER:** rewrite the checklist for the compat-mode reality — both legs are compat per **D8** (the
banner already warns); a GDS-side / Lustre-over-EFA section becomes relevant only if a per-cell path split
ever contradicts D8.
...
nobody reads a sub-interval p50 as a real number. The interval is **1 ms** (ratified 2026-08-21, Stage-7
register): build-machine visibility fell below the earlier 10 ms, which was therefore sampling poll phase;
the recorded floor stays the quantisation guard, and the cell's `sar` shows what the tighter poll costs.
...
### `wsi-lustre-phase2.sh` — the baked lustre mount automation (`D6`/`D16`/`D-17`) — **BAKED; from-scratch proven (2026-08-21 rebuild, zero human steps)**
