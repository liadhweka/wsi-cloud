...
| | |
|---|---|
| **Status** | ✅ Leg A (weka, 16/16 cells OK; per-cell `RECORD_POLL_HZ=10` — the D-34 rate, its non-perturbation proven by the recorded 1-vs-10 Hz pair) · ⏳ Leg B |
| **Leg A results (`s2.0-properties-summary-weka.csv`; app slides/s, n 1→8→64→256)** | **BRCA cold: 47 → 327 → 858 → 700 · warm: 69 → 500 → 1,825 → 1,163. CAM16 cold: 42 → 285 → 579 → 398 · warm: 83 → 602 → 1,489 → 669.** The knee sits at **n=64 on every curve**, with n=256 oversubscribed and declining — the curve shape, not a ceiling claim (Stage 2 reports no ceiling-relative figure by design, item 19). The **cold/warm gap is ~1.5–2× everywhere**: the dentry/attribute caches serve `open()`+header reads client-side, which is exactly why the cold arms drop caches at level 3. High-n cells complete in 0–2 s; their filesystem-side series exist because of the 10 Hz sampling, and app-level rates remain the cross-leg-comparable figures (ops-counter semantics stay within-leg until verified equivalent — this stage's standing caveat). Cache arms reconciled per cell (cold: recorded drop acknowledgment; warm: constructed warmup) |
