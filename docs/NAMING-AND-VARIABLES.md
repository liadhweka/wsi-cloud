...
reason they are enumerated here instead of being left to the scripts.
...
| `EXTRACT_GPUS` | **`0,1,2,3`** | `CUDA_VISIBLE_DEVICES` for the extract workload. The **set** is valid on the 4-GPU instance; the NUMA/NIC-aware **order** is still to be derived from `nvidia-smi topo -m` on the real instance (deferred item `D-8`). The driver refuses any index the instance does not have — because `CUDA_VISIBLE_DEVICES` *drops* an unknown index silently, so the workload would run at half width while the cell claimed full width. **A guard cannot catch a wrong order**, which is why `D-8` still gates. ⚠ **Unresolved: this default overlaps the MIL workload's pinned GPU 0.** `orchestrate-concurrent-stage6c.sh` pins MIL to `CUDA_VISIBLE_DEVICES=0` precisely to stay off the extract workload's GPUs, and 6.C runs its workloads **concurrently** — so any 6.C cell naming both `extract` and `mil` contends on GPU 0, and reports a concurrency figure for a placement nobody chose. **Set `EXTRACT_GPUS` explicitly off GPU 0 for such a cell** until the driver default is resolved with `D-8`. |
| `EXTRACT_N_GPUS` | **`4`** | DDP world size for the extract workload. Must equal the number of indices in `EXTRACT_GPUS`; a mismatch changes the concurrency the cell actually ran at. |
