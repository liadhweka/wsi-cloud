...
| | |
|---|---|
| **Status** | ✅ Leg A (weka, 4/4 cells OK; canary PASS on all; output-count gate exact: **1064 × 3 = 3192 `.pt`**, zero refused) · ⏳ Leg B |
| **Leg A results (`s6.A-extract-summary-<leg>.csv`)** | **cuCIM full-cohort cells (canonical SVS, N=4):** Virchow2 **951**, GigaPath **801**, UNI2-h `[PENDING-APPROVAL]` **960** tiles/s — ~10% *above* their Tier-1 subset comparators (859/729/873), so throughput holds at production scale. Cache `warm` reconciled CONSISTENT. **Multimodel kvikIO chunked cell (6 × ~178-slide chunks, 26.1 h total):** per-model steady extract **1,009 / 850 / 1,030 tiles/s** — at or above the Tier-1 kvikIO peaks; **convert-vs-extract split: 15.9 h convert (re-measuring 4.D's 15.7 h within 1.3% — a free internal-consistency check) vs 10.3 h extract across all three models**, which is the empirical argument for the cross-model conversion sharing (per-model orchestration would have tripled the 15.9 h). Path proof per chunk: `gds_engaged=none` (D8); regime `na-mixed-rw-chunk-resident` → NOT_APPLICABLE by design. **D18 stability (long cell, chunk-window variant): per-chunk extract-rate spread 8.1% / 7.4% / 6.6%** across the 6 chunks. *Caveat:* the multimodel cell's whole-window GPU-utilization (~38%) is diluted by the conversion phases — read the per-model extract rates, not the cell-window mean. |
...
| | |
|---|---|
| **Status** | ✅ Leg A (weka, 9/9 cells OK + 6 D18 rep cells; cache declarations reconciled CONSISTENT on all 15) · ⏳ Leg B — needs its 6.A Tier 2 features |
| **Leg A results (`s6.B-mil-summary-weka.csv`)** | **Slides/s steady, nw 4→16→32:** Virchow2 33.6→37.9→37.8, GigaPath 29.6→33.1→33.1, UNI2-h `[PENDING-APPROVAL]` 30.0→33.5→33.6 — **saturation knee at `num_workers=16` for all three models** (throughput flat 16→32), which is the measured value 6.C's MIL workload consumes (`MIL_NUM_WORKERS=16`, identical on both legs). **D18 reps at each knee: medians 39.0 / 33.1 / 33.5 slides/s with 3.1% / 0.5% / 0.2% spreads.** Regime as the roadmap row predicted: declared `warm` — the 60–72 GB per-model corpus sits far under host RAM, so after the cold first pass the workload is memory-served and these are **MIL-throughput numbers, not storage bandwidth** (the cold storage number belongs to 6.B.2's synthetic corpus). The storage signal: each model's first cell (nw=4, cold first pass) read the corpus at **1.6–1.8 GB/s fs-side active-window mean (peaks 2.0–2.3 GB/s)**; every later cell ran fs-idle — consistency canary PASS on the three first-pass cells, memory-served no-traffic judgement recorded on the rest (the declared regime's expected signature, per the Stage-7 warm-cell sampling-limit rule) |
...
| | |
|---|---|
| **Status** | ✅ Leg A (weka, 12/12 cells OK; declarations reconciled NOT_APPLICABLE by design; canary PASS on every judged direction, REPORT_ONLY exactly where declared) · ⏳ Leg B |
| **Leg A results (`s6.C-concurrent-summary-weka.csv`)** | **Retention vs same-leg solos — the headline: no workload loses more than ~11% under any mix, and the filesystem is never the contended resource.** Extract: **100.2–102.3% in every mix** (fully retained — GPU-bound with storage headroom). Viewer (23.4k IOPS solo, p99 0.2 ms): 97.4–98.4% in pairs/triples; **89.1% with p99 0.7 ms in the 30-min all-four**, recovering to 96.5%/0.2 ms in the endurance window — read with the ingest-active caveat: data-bounded ingest is live for ~⅓ of the short cell but ~3% of the 4-h window, so the all-four figure is the contention-while-ingesting number and the endurance figure is the steady-day number. MIL: 93.7–98.0% in pairs/triples, 92.6% all-four → 98.3% endurance (same caveat; host-CPU contention per D15, not filesystem — fs-side stays far under ceilings throughout). Ingest: 106–114% (no degradation; >100% is solo-window run variance). **Endurance (4 h): D18 split-window — extract −2.1% first-half→second-half; no QoS drift, no leak.** Wire findings recorded with the declarations row below: bulk ingest writes at the 5+2 relation (1.44, PASS) in every mix; feature-trickle writes ~2.6×; reads-under-dominant-write 1.28 (REPORT_ONLY by construction). *Shakedown note (data-validity):* four forensic-renamed dirs record the path here — two canary-criterion gaps (bs-hint, wire-exempt) and the ingest no-op bug — all fixed in code before any kept cell; every kept cell ran the mix its name claims |
...
| **Canary declarations (D12, added 2026-08-22 after two live canary-abort fires)** | 6.C cells declare what their composite traffic is, because the run name cannot: viewer-containing cells declare **`RECORD_BS_HINT`** (`4k` solo / the heterogeneous envelope on combos — the viewer's pure-4K stream measured wire/app 1.414, exactly the 4K calibration, and was falsely FAILed at the large-block band), and extract-without-ingest mixes declare **`RECORD_WIRE_EXEMPT=write:…`** — their only writer is the extractor's torch.save small-buffer trickle, which **measured 2.58× on the wire under 5+2 EC versus 1.46 for bulk writes** (a real amplification with no calibrated analogue; recorded REPORT_ONLY, never judged, and itself a finding: small-buffer writers pay ~1.8× the bulk EC wire cost on this leg). **Ingest-containing mixes declare `read:…`** — a modest read stream under a dominant bulk write stream carries the 4.D-class read amplification (**1.28 measured here; 1.22/1.74 at 4.D**), which the same-file fio mixed probes structurally do not reproduce; the write side stays fully judged at the EC relation, and the 6.C per-workload-sum reconciliation remains the cell's cross-source check |
...
| | |
|---|---|
| **Status** | ✅ Leg A (composed — no cells to run, so the mechanical closeout gate does not apply) · ⏳ Leg B (compose identically at its Stage-6 close) |
| **Leg A results (`s6.D-e2e-composed-summary-weka.csv`; raw SVS → trained MIL classifier, per backend × model, as-run rates dated 2026-08-15)** | **cuCIM path: Virchow2 3.47 h ($157 infra / $184 all-in) · GigaPath 4.11 h · UNI2-h `[PENDING-APPROVAL]` 3.44 h. kvikIO path: 8.54 / 9.14 / 8.47 h ($384–414 infra)** — the conversion share (19,025 s amortised, from the chunked cell's recorded per-chunk convert total of 15.85 h ÷ 3) dwarfs kvikIO's extraction advantage, so **in a single pipeline pass the cuCIM path wins end-to-end ~2.2–2.5× despite extracting ~8% slower**. The crossover logic for the writeup: conversion is one-time per corpus and amortises across models *and* across re-extraction passes — the kvikIO path repays it only in pipelines that re-extract often; this composition (three models, one pass) is already the three-way-shared case. Components: tissue 105 s (3.0 n=64) · extract per model per backend (the Tier-2 cells) · MIL epoch 28.1/32.1/31.8 s (derived — see the recipe). |
...
| **Composition — the ratified recipe (2026-08-23), identical on both legs** | Per (backend, model): **tissue** = the 3.0 TCGA-BRCA **n=64** cell's run window (the measured production-shaped optimum; lower-n cells are curve data, not pipeline configs) + **convert** (kvikIO only) = the chunked Tier-2 cell's **own recorded per-chunk convert total, amortised across the models that share it** (same run, same interleaving as the extraction it composes with — the standalone 4.D figure agrees within 1.3% and stays the conversion *measurement*; this is a provenance choice, not a magnitude one) + **extract** = that model's Tier-2 cell wallclock (kvikIO: the chunked cell's per-model summary; cuCIM: its full-cohort cell) + **MIL one epoch, derived** = `n_slides_in_corpus ÷ slides_per_sec_steady` from the **nw=16 knee cell** (the 6.B.3 cells are fixed-window throughput cells, not epoch runs; startup excluded — the MIL share is ~0.1% of the pipeline). Cost follows from the composed wallclock at the leg contract's recorded rates; headline costs come from the publication-time reprice |
| **Cells executed** | **0** |
| **Tools** | **`../scripts/compose-stage6d.py` — the recipe as code, so both legs compose identically** · `../scripts/pipeline-end-to-end-stage6d.sh` — retained as the productisation template · `../scripts/aggregate-stage6d.py` (aggregates the live variant only, if productisation ever runs it) |
